...
- **`COOKIE_SAMESITE` stays `none`** while frontend and API are on different domains. Only switch to `lax` after a custom domain makes them same-site (`frankly.app` + `api.frankly.app`). `none` requires `ENV=prod` (the Secure flag follows `ENV`).
