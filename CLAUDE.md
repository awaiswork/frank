...
- **A transfer is one row, and every aggregate must stay blind to it.** `kind = 'transfer'` with `counter_account_id`, guarded by `ck_transactions_transfer_shape` — both ends present, different, and no category, so it can never reach a budget however the row is written. It costs nothing to exclude because all six spend aggregates filter `kind` *positively* (`== 'expense'`, `== 'income'`); there is not one `!=` or `NOT IN` in the codebase, and there must not be. Two tests hold this: conservation (`Σ(balances)` cannot move when money is moved between your own accounts) and aggregate invariance (every spending figure identical with transfers present).
