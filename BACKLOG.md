...
| Month-over-month `LAG` is not calendar-aware | `LAG` ordered over *rows that exist*, so "previous month" meant "the previous month with spend in that category" — and rows were filtered to the target month, so a category you stopped buying vanished instead of showing a fall to zero. | **Fixed** — two reads over the union of both months. The window function is gone; correctness over the showcase query, noted in the docstring. |
